...
bool dcsr_csr_t::unlogged_write(const reg_t val, processor_t* proc) noexcept {
...
void stimecmp_csr_t::verify_permissions(insn_t insn, bool write, processor_t* p) const {
  state_t* state = p->get_state();
  if (!(state->menvcfg->read(p) & MENVCFG_STCE)) {
    // access to (v)stimecmp with MENVCFG.STCE = 0
    if (state->prv < PRV_M)
      throw trap_illegal_instruction(insn.bits());
  }

  state->time_proxy->verify_permissions(insn, false, p);

  if (state->v && !(state->henvcfg->read(p) & HENVCFG_STCE)) {
